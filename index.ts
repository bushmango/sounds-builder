...
function onWatchEvent(loc, type, isMusic) {
  ...
  if (mode === "convert" || mode === "all" || isMusic) {
  ...
  if ((mode === "sprite" || mode === "all") && !isMusic) {
...
function createSprite() {
  ...
  fs.readdir(path, (err, items) => {
    ...
    console.log("creating audiosprite");
    audiosprite(files, opts, (err, obj) => {
      console.log("audiosprite", obj);
      ...
      let file = outPath + "audioSprite1.json"; // 'audioSprite.txt'
